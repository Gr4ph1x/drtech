<?php
    ...
    if (isset($_POST["btnUpdate"])) {
        ...
        if ($stmt = $con->prepare("UPDATE `products` SET `product`=?, `specs`=?, `description`=? WHERE `id`=?")) {
    ...
    if ($stmt = $con->prepare("SELECT `product`, `specs`, `description` FROM `products` WHERE `id`=? LIMIT 1")) {
